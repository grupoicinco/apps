	  ...
	  */
	  public function plugin_disc_tests($passcode = NULL, $id){
		...
		if($tipo->TEST_TYPE == 'Empleado' && $validation):
			$query = $this->db->select("PDT.ID, PSM.ID AS USERID, PDT.TEST_PASSCODE, PDT.TEST_MASANSWERS, PDT.TEST_MENOSANSWERS, PDT.TEST_EXPDATE, PSM.SALESMAN_SAC_CODE AS CODE, PSM.SALESMAN_NAME AS NAME, PSM.SALESMAN_LASTNAME AS LASTNAME, PSM.SALESMAN_DPI AS IDNUMBER, PSM.SALESMAN_BIRTHDATE AS BIRTHDATE")
